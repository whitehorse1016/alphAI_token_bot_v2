...
export const getFilteredUsers = (web3, tokenInfo, usersInDb) => {
  ...
  for (const user of usersInDb) {
    if (user.wallet || user.vip === 1) {
      if (
        tokenInfo.secondaryAddress.toLowerCase() ==
        uniconst.WETH_ADDRESS.toLowerCase()
      ) {
        tokenInfo.initialLiquidity = parseFloat(
          web3.utils.fromWei(tokenInfo.secondaryAmount, "ether")
        );
        if (
          !user.init_eth ||
          tokenInfo.initialLiquidity < parseFloat(user.init_eth)
        ) {
          continue;
        }
      } else if (
        tokenInfo.secondaryAddress.toLowerCase() ==
          uniconst.USDT_ADDRESS.toLowerCase() ||
        tokenInfo.secondaryAddress.toLowerCase() ==
          uniconst.USDC_ADDRESS.toLowerCase()
      ) {
        tokenInfo.initialLiquidity = parseFloat(
          web3.utils.fromWei(tokenInfo.secondaryAmount, "mwei")
        );
        if (
          !user.init_usd ||
          tokenInfo.initialLiquidity < parseFloat(user.init_usd)
        ) {
          continue;
        }
      } else {
...
export const getScamInfo = (web3, tokenDetailInfo, checksumForContract) => {
  ...
  if (scamCount === 0) {
    if (totalTermCount > 0) {
      ...
      result += `\n💀 Scam Detection: ${scamChecksum} (${utils.roundDecimal(
        percent,
        3
      )} % scam) | Total ${totalTermCount} | Scams ${scamTermCount}`;
    }
  } else {
    if (totalCount > 0) {
      ...
      result += `\n💀 Scam Detection: ${scamChecksum} (${utils.roundDecimal(
        percent,
        3
      )} % scam) | Total ${totalCount} | Scams ${scamCount}`;
    }
  }
...
async function getAddressInfo(web3, address) {
  ...
  const transactionCountPromise = web3.eth
    .getTransactionCount(address)
    .then(
      (transactionCount) => (addressInfo.transactionCount = transactionCount)
    );

  const balancePromise = web3.eth
    .getBalance(address)
  ...
  const wethBalancePromise = wethTokenContract.methods
    .balanceOf(address)
    .call()
  ...
  const usdtTokenContract = new web3.eth.Contract(
    ERC20_ABI,
    uniconst.USDT_ADDRESS
  );
  const usdtBalancePromise = usdtTokenContract.methods
    .balanceOf(address)
    .call()
  ...
  const usdcTokenContract = new web3.eth.Contract(
    ERC20_ABI,
    uniconst.USDC_ADDRESS
  );
  const usdcBalancePromise = usdcTokenContract.methods
    .balanceOf(address)
    .call()
...
const getSecondaryTokenPrice = async (web3, tokenAddress) => {
  if (
    tokenAddress.toLowerCase() === uniconst.USDT_ADDRESS.toLowerCase() ||
    tokenAddress.toLowerCase() === uniconst.USDC_ADDRESS.toLowerCase()
  ) {
    return 1;
  } else if (
    tokenAddress.toLowerCase() === uniconst.WETH_ADDRESS.toLowerCase()
  ) {
...
export const checkLPStatus = async (web3, poolAddress) => {
  ...
  let result = {
    success: false,
    lpLocked: false,
    contractOwnedPercent: 0,
    contractOwnedAmount: 0,
  };
  const resp = await utils.fetchAPI(
    `${apiUrl}TopHolders?address=${poolAddress}&chainID=1`
  );
  ...
  for (const holder of resp.holders) {
    ...
    if (holder.address === uniconst.NULL_ADDRESS) {
      ...
      if (lowerAddr == uniconst.UNICRYPT_CONTRACT_ADDRESS.toLowerCase()) {
        ...
        lowerAddr == uniconst.PINKLOCK_CONTRACT_ADDRESS.toLowerCase()
      ) {
        resp = await utils.getPinkLockDetails(web3, poolAddress);
        lockerName = "PinkLock";
      } else if (
        lowerAddr == uniconst.TEAMFINANCE_CONTRACT_ADDRESS.toLowerCase()
      ) {
      ...
      if (resp) {
        ...
        node.msg = `${utils.roundDecimal(
          node.percent
        )}% of LP Locked for ${span} days left via ${lockerName}`;
        result.lpLocked = true;
      } else {
        node.msg = `${lockerName}`;
      }
...
export const getTokensInContract = async (web3, tokenAddress) => {
  ...
  const resp = await utils.fetchAPI(
    `${apiUrl}TopHolders?address=${tokenAddress}&chainID=1`
  );
...
export const checkHoneypot = async (tokenAddress) => {
  ...
  if (resp.simulationResult) {
    message += `\n     └─ Buy Tax: ${utils.roundDecimal(
      resp.simulationResult.buyTax,
      1
    )}%`;
    message += `\n     └─ Sell Tax: ${utils.roundDecimal(
      resp.simulationResult.sellTax,
      1
    )}%`;
...
export const checkDormantWallet = async (walletAddress, minDormantDuration) => {
  try {
    const timeStamp = await utils.getLastTransactionDateFromAddress(
      walletAddress
    );
    if (!timeStamp) {
      return false;
...
export const start = async (web3, tokenInfo, filterCriteria, sniper) => {
  ...
  try {
    ...
    const primaryContract = new web3.eth.Contract(
      ERC20_ABI,
      tokenInfo.primaryAddress
    );
    const secondaryContract = new web3.eth.Contract(
      ERC20_ABI,
    ...
    console.log(
      "First swap event emitted. Waiting for " +
        filterCriteria.blockThreshold +
        " blocks confirmation..."
    );

    let block0Liquidity = await secondaryContract.methods
      .balanceOf(tokenInfo.poolAddress)
      .call();
    ...
    for (const purchaseAddress of purchaseAddresses) {
      const filterAddress = async () => {
        ...
        const isFresh =
          ...
          filterCriteria.maxFreshTransactionCount;

        const totalBalance =
          ((Number(addressInfo.balance) + Number(addressInfo.wethBalance)) *
            Number(ethPrice)) /
            10 ** 18 +
          (Number(addressInfo.usdtBalance) + Number(addressInfo.usdcBalance)) /
            10 ** 6;
        const isWhale = totalBalance >= filterCriteria.minWhaleBalance;

        const primaryBalance = await primaryContract.methods
          .balanceOf(purchaseAddress)
          .call();

        let isDormant = false;

        if (
          filterCriteria.minDormantWalletCount &&
          filterCriteria.minDormantWalletCount > 0 &&
          dormantWalletCount < filterCriteria.minDormantWalletCount
        ) {
          isDormant = await checkDormantWallet(
            purchaseAddress,
            filterCriteria.minDormantDuration
          );
        }
    ...
    if (
      filterCriteria.minFreshWalletCount &&
      freshWalletCount < filterCriteria.minFreshWalletCount
    ) {
      console.log(
        `Fresh wallet count is ${freshWalletCount} which is less than ${filterCriteria.minFreshWalletCount}`
      );
      return;
    }

    if (
      filterCriteria.minWhaleWalletCount &&
      whaleWalletCount < filterCriteria.minWhaleWalletCount
    ) {
      console.log(
        `Whale wallet count is ${whaleWalletCount} which is less than ${filterCriteria.minWhaleWalletCount}`
      );
      return;
    }
    ...
    if (
      ...
      console.log(
        `Dormant wallet count is ${dormantWalletCount} which is less than ${filterCriteria.minDormantWalletCount}`
      );
      return;
    }
    ...
    if (kycWalletCount < filterCriteria.minKycWalletCount) {
      console.log(
        `KYC wallet count is less than ${filterCriteria.minKycWalletCount}`
      );
      return;
    }
    ...
    let currentPrimaryAmount = await primaryContract.methods
      .balanceOf(tokenInfo.poolAddress)
      .call();
    ...
    const contractAgeStat = await checkContractAge(
      web3,
      tokenInfo.primaryAddress
    );

    if (
      filterCriteria.contractAge > 0 &&
      contractAgeStat.contractAge < filterCriteria.contractAge
    ) {
    ...
    const checksumForContract = await utils.getContractVerified(
      web3,
      tokenInfo.primaryAddress
    );
    ...
    let tokensInContractStat = await getTokensInContract(
      tokenInfo.primaryAddress
    );
    ...
    if (tokensInContractStat.success) {
      tokensInContractPercent = utils.roundDecimal(
        tokensInContractStat.contractOwnedPercent,
        1
      );
      tokensInContractAmount = utils.roundDecimal(
        tokensInContractStat.contractOwnedAmount /
          10 ** Number(primaryInfo.decimals)
      );
    }
    ...
    if (result.purchaseCount > 0) {
      sniperOwnedPercent =
        (result.sniperPurchaseCount * 100) / result.purchaseCount;
    }
    ...
    // get top holders
    const tokenDetailInfo = await apiRepeater.getTokenDetailInfo(
      tokenInfo.primaryAddress
    );
    let topHoldersResult = await apiRepeater.getTopHolders(
      tokenInfo.primaryAddress,
    ...
    if (
      tokenDetailInfo &&
      tokenDetailInfo.links &&
      tokenDetailInfo.links.length > 0
    ) {
      socialMsg = "\n\n<u>Social Info</u>";
    ...
    if (tokenDetailInfo && tokenDetailInfo.ownership) {
      ownershipRenouncedMsg =
        "\n🔍 Ownership Renounced: " +
        (tokenDetailInfo.ownership.renounced ? "Yes" : "No");
    }
    ...
    let currentSecondaryAmount = await secondaryContract.methods
      .balanceOf(tokenInfo.poolAddress)
      .call();
    currentSecondaryAmount = Number(currentSecondaryAmount);

    const primaryPriceBySecondary =
      (currentSecondaryAmount / currentPrimaryAmount) *
      10 ** (Number(primaryInfo.decimals) - Number(secondaryInfo.decimals));
    const secondaryPriceByUSD = await getSecondaryTokenPrice(
      web3,
      tokenInfo.secondaryAddress
    );
    ...
    const initialLiquidity =
      tokenInfo.secondaryAmount / 10 ** Number(secondaryInfo.decimals);
    const initialLiquidityFund = initialLiquidity * secondaryPriceByUSD;

    let currentLiquidity = await secondaryContract.methods
      .balanceOf(tokenInfo.poolAddress)
      .call();
    ...
    const block0Volume =
      (Number(block0Liquidity) * secondaryPriceByUSD) /
      10 ** Number(secondaryInfo.decimals);

    const totalSupply = await primaryContract.methods.totalSupply().call();
    const marketCap =
      (Number(totalSupply) * primaryPriceByUSD) /
      10 ** Number(primaryInfo.decimals);
    ...
    const content0 = `⚡ Name: ${primaryInfo.name} (${primaryInfo.symbol})
💧 Initial Liquidity: ${roundDecimal(initialLiquidity, 3)} ${
      secondaryInfo.symbol
    } ($ ${roundDecimal(initialLiquidityFund, 2)})
🏠 Token address: <code class="text-entity-code">${
      tokenInfo.primaryAddress
    }</code>
...
${honeypotMsg}
${contractAgeMsg}
📊 Chart: <a href="https://dexscreener.com/ethereum/${
      tokenInfo.primaryAddress
    }">Dexscreener</a> | <a href="https://www.dextools.io/app/en/ether/pair-explorer/${
      tokenInfo.poolAddress
    }">DexTools</a>`;
...
🏦 Balance in contract: ${tokensInContractAmount} ${
      primaryInfo.symbol
    } own ${tokensInContractPercent}%
